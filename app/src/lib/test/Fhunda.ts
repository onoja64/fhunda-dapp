...
describe("Fhunda - Fully Private FHE Crowdfunding with fheUSDT", () => {
  ...
  describe("Campaign Creation", () => {
    it("creates a campaign with encrypted target", async () => {
      ...
      await fhunda.connect(creator).createCampaign(encTarget.handle, encTarget.inputProof, CAMPAIGN_DURATION);
    ...
    it("rejects zero duration", async () => {
      ...
      const encTarget = await encryptValue64(creator, fhundaAddress, TARGET_AMOUNT);
      await expect(
        fhunda.connect(creator).createCampaign(encTarget.handle, encTarget.inputProof, 0),
  ...
  describe("End-to-End Lifecycle", () => {
    ...
    it("handles multiple campaigns simultaneously", async () => {
      ...
      await fhunda.connect(creator).createCampaign(encTarget1.handle, encTarget1.inputProof, CAMPAIGN_DURATION);

      // Create second campaign
      const encTarget2 = await encryptValue64(creator, fhundaAddress, TARGET_AMOUNT);
      await fhunda.connect(creator).createCampaign(encTarget2.handle, encTarget2.inputProof, CAMPAIGN_DURATION);
